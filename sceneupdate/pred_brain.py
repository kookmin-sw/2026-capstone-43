"""
PREDBrain - PRED-based task planner with programmatic DTA/APM/ASR

PRED = Pre-emptive action Revision by Environmental Feedback
Key mechanisms (implemented as code, not just LLM prompts):
...
from config import EDGE_JSON_FILE, HIERARCHY_JSON_FILE, SURFACE_JSON_FILE
...
class PREDBrain:
    def __init__(self, json_path, react_bridge=None, llm=None):
        ...
        if os.path.exists(SURFACE_JSON_FILE):
            ...
            print(f"[PRED] Loaded surface JSON: {SURFACE_JSON_FILE}")
    ...
    def _load_hierarchy_and_edges(self):
        ...
        if os.path.exists(HIERARCHY_JSON_FILE):
            ...
            print(f"[PRED] Loaded hierarchy: {len(self.room_map)} objects in rooms: "
        ...
        if os.path.exists(EDGE_JSON_FILE):
            try:
                ...
                print(f"[PRED] Loaded {len(self.edges)} edge relations")
            except Exception as e:
                print(f"[PRED] Edge load error: {e}")
    ...
    def check_false_positive_reverts(self):
        ...
        for key, obj in self.raw_data.items():
            if obj.get("_is_new") and str(obj.get("id")) not in bridge_ids:
                ...
                print(f"[PRED] False positive reverted: '{tag}'(id:{obj.get('id')})")
                reverted.append(tag)
    ...
    def _find_substitute(self, target_id, category_hint=None):
        ...
        print(f"[PRED-DTA] Target '{target_name}'(id={target_id}) "
    ...
    def _check_apm(self, plan_actions):
        ...
        while i < len(plan_actions):
            ...
            # Check place() actions
                ...
                if any(ct in dest_tag for ct in _CONTAINER_TAGS):
                    plan_so_far = " ".join(new_plan)
...
"""
        try:
            ...
            print(f"[PRED] LLM guessed likely locations for '{target_name}': {guessed_ids}")
            return guessed_ids[:num_targets]
        except Exception as e:
            print(f"[PRED] Error guessing search locations: {e}")
    ...
    def _revise_plan(self, plan_actions, user_instruction):
        ...
        if revision_log:
            print(f"[PRED] Plan revised with {len(revision_log)} modifications:")
    ...
    def _build_system_prompt(self):
        ...
        return f"""You are a robot task planner controlling a mobile manipulator (Jackal base + Franka arm).
PRED = Pre-emptive action Revision by Environmental Feedback.
...
Actions:
- goto(ID): Move robot to object. MUST do before pick/place/open/close.
...
- done(): Task complete.
- expand_node(ROOM): Reveal objects in a room. MUST do first if graph is collapsed.

Rules:
1. If graph is collapsed, you MUST expand_node a room first.
...
5. If duplicates exist, system auto-selects closest one.
6. System auto-corrects: skips redundant actions (ASR), substitutes missing targets (DTA), inserts open/close (APM).

You MUST respond with valid JSON in one of these formats:

FORMAT 1 - Exploring (when graph is collapsed):
{{"chain_of_thought": "Graph is collapsed. Need to expand room to see objects.", "mode": "exploring", "command": {{"command_name": "expand_node", "node_name": "{room_list[0]}"}}}}

FORMAT 2 - Planning (when graph is expanded):
{{"chain_of_thought": "Step by step reasoning about which objects to use...", "mode": "planning", "command": {{"command_name": "plan", "plan": ["goto(13)", "pick(13)", "goto(9)", "place(9)", "done()"]}}}}

EXAMPLES:

User says "move the apple to the plate" (graph expanded, apple id=5, plate id=12):
{{"chain_of_thought": "Apple is id 5, plate is id 12. Go to apple, pick it, go to plate, place on it.", "mode": "planning", "command": {{"command_name": "plan", "plan": ["goto(5)", "pick(5)", "goto(12)", "place(12)", "done()"]}}}}

User says "put the book on the desk" (graph collapsed):
{{"chain_of_thought": "Graph is collapsed. Must expand room first to see objects.", "mode": "exploring", "command": {{"command_name": "expand_node", "node_name": "{room_list[0]}"}}}}

User says "move cup to table" (already holding cup id=7, table id=3):
{{"chain_of_thought": "Already holding cup. Just go to table and place.", "mode": "planning", "command": {{"command_name": "plan", "plan": ["goto(3)", "place(3)", "done()"]}}}}
"""
...
Respond as JSON: {{"objects": ["cup", "table"]}}
"""
        try:
            resp = self.model.generate(prompt)
            text = resp.text.replace("```json", "").replace("```", "").strip()
            ...
            print(f"[PRED] Extracted objects from instruction: {result}")
            return result
        except Exception as e:
            print(f"[PRED] Object extraction error: {e}")
            return []
    ...
    def process_turn(self, user_instruction, simulator_feedback=""):
        ...
        if (not is_replanning
            and self.current_graph_state == self.full_graph_str
...
Respond as JSON: {{"answer": "PROCEED"}} or {{"answer": "BLOCKED"}}"""
                        try:
                            ...
                            if "BLOCKED" in resp_text:
                                ...
                                print(f"[PRED] Some objects missing ({missing}) but can PROCEED with available: {available}")
                        except Exception as e:
                            print(f"[PRED] Dependency check error: {e}")
                            should_search = True

                    if should_search:
                        missing_name = missing[0]  # Handle first missing object
                        print(f"[PRED] Task blocked. Entering search mode for '{missing_name}'.")
...
"""
            ...
            if parsed_resp.get("mode") == "planning":
                ...
                if original_plan:
                    ...
                    if revision_log:
                        ...
                        print(f"[PRED] Original: {original_plan}")
                        print(f"[PRED] Revised:  {revised_plan}")
    ...
    def get_placement_pos(self, surface_id):
        """Find the best free-space placement position on a surface.

        Uses SurfaceAnalyzer to find the optimal position that avoids
        existing objects. Falls back to surface center if no free space found.
        If target is not a surface (e.g. plate), finds the parent surface.
        ...
        # If target is NOT a surface, find parent surface
        if sid not in self.surface_analyzer.surfaces:
            ...
            if parent_surface_id is not None:
                ...
                print(f"[PRED] place target '{tag}'(id:{sid}) is not a surface, "
                      f"using parent surface '{parent_tag}'(id:{parent_surface_id})")
                near_obj_id = sid
                sid = parent_surface_id
            else:
                print(f"[PRED] place target id:{sid} is not a surface and no parent found")

        # Get held object size for fitting check
        obj_w, obj_d = 0.15, 0.15
        if self.holding_object is not None:
            for info in self.raw_data.values():
                if str(info.get("id")) == str(self.holding_object):
                    ext = info.get("bbox_extent", [0.15, 0.15, 0.1])
                    obj_w, obj_d = ext[0], ext[1]
                    break
        ...
        if placement is not None:
            ...
            print(f"[PRED] Placement on surface {sid}: "
        ...
        if center is not None:
            print(f"[PRED] Placement fallback: surface {sid} center")
    ...
    def _find_parent_surface(self, obj_id):
        ...
        # 2) Proximity: find closest surface near the object
        obj_pos = self.get_coords_by_id(obj_id)
        if obj_pos is None:
            return None
        best_sid, best_dist = None, float("inf")
        for s_id, surf in self.surface_analyzer.surfaces.items():
    ...
    def _compute_spatial_hints(self, obj1_info, obj2_info):
        ...
        on_gap = abs(obj1_bottom - obj2_top)
...
{{"object_relation": "one of the above", "reason": "brief explanation"}}"""
        ...
        try:
            ...
            print(f"[PRED] LLM edge: {tag1} --[{relation}]--> {tag2} ({reason})")
            return relation
        except Exception as e:
            print(f"[PRED] LLM edge generation failed: {e}")
            return None
    ...
    def generate_edges_for_new_objects(self):
        ...
        if new_edge_count > 0:
            ...
            print(f"[PRED] Generated {new_edge_count} new edges for REACT objects")
    ...
    def _save_edges_json(self):
        ...
        try:
            ...
            print(f"[PRED] Saved {len(self.edges)} edges to {EDGE_JSON_FILE}")
        except Exception as e:
            print(f"[PRED] Failed to save edges: {e}")
