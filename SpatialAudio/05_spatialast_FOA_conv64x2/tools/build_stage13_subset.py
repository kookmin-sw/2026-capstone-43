...
OUTPUT_ROOT = ROOT / "manifests_stage13"
...
def write_summary(path, split_stats):
    with open(path, "w") as f:
        f.write("# Stage-13 Subset Summary\n\n")
        f.write("Subset goal: compare deeper FOA-native stem variants under the same slow recipe.\n")
        f.write("Hypothesis: a 3-block stem such as 7 -> 64 -> 64 -> out may preserve and mix FOA cues better before patch embedding.\n")
...
def main():
    ...
    train_path = output_dir / f"train_stage13_subset_{args.train_size}.json"
    val_path = output_dir / f"val_stage13_subset_{args.val_size}.json"
    write_json(train_path, train_stats["data"])
    write_json(val_path, val_stats["data"])

    summary_path = output_dir / "stage13_subset_summary.md"
