...
MANIFEST_ROOT="${MANIFEST_ROOT:-${PROJECT_ROOT}/manifests_stage15/hm3d_losnlos_100k_balanced_world_full360}"
...
EPOCHS="${EPOCHS:-30}"
SEED="${SEED:-1337}"
RUN_NAME="${RUN_NAME:-world_full360_sincos_azonly_${STEM_VARIANT}_last${UNFREEZE_LAST_N_BLOCKS}_$(date +%Y%m%d_%H%M%S)}"
...
if [ ! -f "${TRAIN_JSON}" ] || [ ! -f "${VAL_JSON}" ]; then
  echo "[stage15-world-azonly] world full360 manifests missing, building them first"
  "${PYTHON_BIN}" "${PROJECT_ROOT}/tools/build_hm3d_losnlos_world_full360_manifests.py" \
...
if [ -d "${OUTPUT_DIR}" ] && [ -n "$(find "${OUTPUT_DIR}" -mindepth 1 -print -quit)" ]; then
  if [ "${OVERWRITE_OUTPUT_DIR}" = "1" ]; then
    ...
    echo "[stage15-world-azonly] output dir already exists and is not empty: ${OUTPUT_DIR}" >&2
    echo "[stage15-world-azonly] set OVERWRITE_OUTPUT_DIR=1 or choose a new RUN_NAME" >&2
    exit 1
  fi
fi
mkdir -p "${OUTPUT_DIR}"
...
args=(
  ...
  --epochs "${EPOCHS}"
  --seed "${SEED}"
  --recipe_name world_full360_azimuth_only
...
echo "[stage15-world-azonly] train_json=${TRAIN_JSON}"
echo "[stage15-world-azonly] val_json=${VAL_JSON}"
echo "[stage15-world-azonly] output_dir=${OUTPUT_DIR}"
if [ -n "${INIT_CHECKPOINT}" ]; then
  echo "[stage15-world-azonly] init_checkpoint=${INIT_CHECKPOINT}"
fi
