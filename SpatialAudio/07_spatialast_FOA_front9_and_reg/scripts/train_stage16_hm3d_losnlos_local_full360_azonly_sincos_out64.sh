...
OUTPUT_ROOT="${OUTPUT_ROOT:-${PROJECT_ROOT}/outputs_stage16}"
...
EPOCHS="${EPOCHS:-30}"
SEED="${SEED:-1337}"
RUN_NAME="${RUN_NAME:-stage16_local_full360_glos_sincos_azonly_${STEM_VARIANT}_last${UNFREEZE_LAST_N_BLOCKS}_$(date +%Y%m%d_%H%M%S)}"
...
if [ ! -f "${TRAIN_JSON}" ] || [ ! -f "${VAL_JSON}" ]; then
  echo "[stage16-local-azonly-out64] local full360 manifests missing, building them first"
...
if [ -d "${OUTPUT_DIR}" ] && [ -n "$(find "${OUTPUT_DIR}" -mindepth 1 -print -quit)" ]; then
  if [ "${OVERWRITE_OUTPUT_DIR}" = "1" ]; then
    ...
    echo "[stage16-local-azonly-out64] output dir already exists and is not empty: ${OUTPUT_DIR}" >&2
    echo "[stage16-local-azonly-out64] set OVERWRITE_OUTPUT_DIR=1 or choose a new RUN_NAME" >&2
    exit 1
  fi
fi
mkdir -p "${OUTPUT_DIR}"
...
args=(
  ...
  --epochs "${EPOCHS}"
  --seed "${SEED}"
  --recipe_name stage16_local_full360_azimuth_only_out64
...
echo "[stage16-local-azonly-out64] train_json=${TRAIN_JSON}"
echo "[stage16-local-azonly-out64] val_json=${VAL_JSON}"
echo "[stage16-local-azonly-out64] output_dir=${OUTPUT_DIR}"
echo "[stage16-local-azonly-out64] stem_variant=${STEM_VARIANT}"
echo "[stage16-local-azonly-out64] batch_size=${BATCH_SIZE}"
if [ -n "${INIT_CHECKPOINT}" ]; then
  echo "[stage16-local-azonly-out64] init_checkpoint=${INIT_CHECKPOINT}"
fi
