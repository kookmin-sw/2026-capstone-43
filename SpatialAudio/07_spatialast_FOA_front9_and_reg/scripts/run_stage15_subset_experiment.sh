...
MANIFEST_ROOT="${PROJECT_ROOT}/manifests_stage15"
TRAIN_MANIFEST="${MANIFEST_ROOT}/train_stage15_subset_600.json"
VAL_MANIFEST="${MANIFEST_ROOT}/val_stage15_subset_150.json"

mkdir -p "${OUTPUT_DIR}"

if [ ! -f "${TRAIN_MANIFEST}" ] || [ ! -f "${VAL_MANIFEST}" ]; then
  echo "[stage15] subset manifests missing, building them first"
  "${PYTHON_BIN}" "${PROJECT_ROOT}/tools/build_stage15_subset.py"
fi
