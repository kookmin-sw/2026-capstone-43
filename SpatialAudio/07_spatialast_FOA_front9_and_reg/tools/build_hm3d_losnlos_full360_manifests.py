...
DEFAULT_OUTPUT_ROOT = ROOT / "manifests_stage15" / "hm3d_losnlos_100k_balanced_full360"
...
def load_entries(dataset_root, require_in_fov):
...
def main():
    ...
    entries = load_entries(dataset_root, require_in_fov=args.require_in_fov)
    ...
    for offset, split_name in enumerate(("train", "val", "test")):
        if split_name not in assignments:
            continue
        split_data = flatten_split(scene_entries, assignments[split_name], seed=args.seed + offset)
        split_payloads[split_name] = split_data
        split_summaries[split_name] = summarize_split(split_name, split_data, assignments[split_name])
    ...
    summary = {
        ...
        "seed": args.seed,
        "require_in_fov": bool(args.require_in_fov),
        "splits": split_summaries,
        "scene_split_assignments": {name: sorted(scene_ids) for name, scene_ids in assignments.items()},
    }
    write_json(output_dir / "summary.json", summary)

    summary_md = output_dir / "summary.md"
    with open(summary_md, "w") as f:
        f.write(f"# HM3D LOS/NLOS Full360 Manifest Summary\n\n")
        f.write(f"- Dataset root: `{dataset_root}`\n")
        f.write(f"- Source manifest: `{summary['source_manifest']}`\n")
        f.write(f"- Require in FOV: `{summary['require_in_fov']}`\n")
        for split_name in ("train", "val", "test"):
            if split_name not in split_summaries:
                continue
            split = split_summaries[split_name]
            f.write(
                f"- {split_name}: {split['sample_count']} samples, {split['scene_count']} scenes, "
                f"LOS={split['los_counts'].get('LOS', 0)}, NLOS={split['los_counts'].get('NLOS', 0)}\n"
            )

    print(f"saved manifests to {output_dir}")
    for split_name, split in split_summaries.items():
        print(
            f"{split_name}: {split['sample_count']} samples, {split['scene_count']} scenes, "
            f"LOS={split['los_counts'].get('LOS', 0)}, NLOS={split['los_counts'].get('NLOS', 0)}"
        )
