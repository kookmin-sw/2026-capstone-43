...
from losses import (
    azimuth_raw_to_front9_class,
    azimuth_raw_to_signed_front_deg,
    azimuth_to_signed_deg,
    azimuth_sincos_to_deg,
    build_front9_label_space_from_manifest,
    compute_losses,
    front9_class_to_signed_deg,
    labels_to_unit_vectors,
)
...
def load_frozen_checkpoint(model, checkpoint_path):
    ...
    full_model_matches = sum(1 for name in state.keys() if name in model_state)
    backbone_matches = sum(1 for name in state.keys() if name in backbone_state)

    if full_model_matches > 0 and full_model_matches >= backbone_matches:
        target_module = model
        target_state = model_state
    else:
        target_module = model.backbone
        target_state = backbone_state
...
def move_batch_to_device(batch, device):
    ...
    if "azimuth_target_deg" in batch:
        moved["azimuth_target_deg"] = batch["azimuth_target_deg"].to(device, non_blocking=True)
...
def signed_deg_to_front9_class(pred_deg, front9_label_space):
    pred_deg = torch.as_tensor(pred_deg).float()
    class_to_signed = torch.as_tensor(
        front9_label_space["class_to_signed"],
        dtype=torch.float32,
        device=pred_deg.device,
    )
    distance = (pred_deg.unsqueeze(-1) - class_to_signed.unsqueeze(0)).abs()
    return distance.argmin(dim=-1)


def get_azimuth_predictions_deg(outputs, args, front9_label_space):
    if args.azimuth_head_mode == "front_regression" and "azimuth_regression" in outputs:
        return outputs["azimuth_regression"].float()
    if args.azimuth_head_mode in {"full360_regression", "full360_sincos_regression"} and "azimuth_regression" in outputs:
        azimuth_output = outputs["azimuth_regression"]
        if azimuth_output.ndim >= 2 and azimuth_output.shape[-1] == 2:
            return azimuth_sincos_to_deg(azimuth_output)
        return azimuth_output.float()
    if args.azimuth_head_mode == "front9_classification" and "azimuth_front9_logits" in outputs:
        front_class = outputs["azimuth_front9_logits"].argmax(dim=-1)
        return front9_class_to_signed_deg(front_class, front9_label_space)
    if "azimuth_logits" in outputs:
        return outputs["azimuth_logits"].argmax(dim=-1).float()
    return None


def summarize_mean_std(sum_value, sq_sum_value, count):
    if count <= 0:
        return 0.0, 0.0
    mean = sum_value / count
    variance = max(sq_sum_value / count - mean * mean, 0.0)
...
def is_better_metric(summary, best_summary, select_best_by):
    ...
    if select_best_by == "azimuth_mae":
        return summary["val_azimuth_mae_deg"] < best_summary["val_azimuth_mae_deg"]
    if select_best_by == "azimuth_loss":
        return summary["val_azimuth_loss"] < best_summary["val_azimuth_loss"]
    if select_best_by == "loss":
        return summary["val_loss"] < best_summary["val_loss"]
...
def freeze_for_stage4(model, args):
    ...
    if hasattr(model.heads, "azimuth_head") and model.heads.azimuth_head is not None:
        for param in model.heads.azimuth_head.parameters():
            param.requires_grad = True
    if hasattr(model.heads, "azimuth_front9_head") and model.heads.azimuth_front9_head is not None:
        for param in model.heads.azimuth_front9_head.parameters():
            param.requires_grad = True
    if hasattr(model.heads, "azimuth_regression_head") and model.heads.azimuth_regression_head is not None:
        for param in model.heads.azimuth_regression_head.parameters():
            param.requires_grad = True
...
def set_stage4_mode(model, train, args):
    ...
    if train:
        ...
        if hasattr(model.heads, "azimuth_front9_head") and model.heads.azimuth_front9_head is not None:
            model.heads.azimuth_front9_head.train()
        if hasattr(model.heads, "azimuth_regression_head") and model.heads.azimuth_regression_head is not None:
            model.heads.azimuth_regression_head.train()
        if hasattr(model.heads, "elevation_head") and model.heads.elevation_head is not None:
...
def collect_optimizer_param_groups(model, args):
    ...
    for maybe_group in [
        ...
        take("patch_embed", "backbone.patch_embed.", args.lr_patch_embed),
    ]:
...
def prepare_debug_targets(batch, train, step):
    ...
    if step == 0 and train:
        ...
        if torch.all((azimuth_target <= 45) | (azimuth_target >= 315)):
            signed_front = azimuth_raw_to_signed_front_deg(azimuth_target)
            print("signed front azimuth sample:", signed_front[:20])


def run_epoch(model, loader, optimizer, device, loss_weights, args, front9_label_space, train=True, max_steps=0):
    ...
    stats = {
        ...
        "vector_loss_sum": 0.0,
        "azimuth_correct": 0,
        "azimuth_front9_correct": 0,
        "azimuth_rounded_front9_correct": 0,
        "elevation_correct": 0,
        "vector_cosine_sum": 0.0,
        "sample_count": 0,
        "azimuth_count": 0,
        "azimuth_front9_count": 0,
        "azimuth_rounded_front9_count": 0,
        "elevation_count": 0,
        "vector_count": 0,
        "azimuth_mae_sum": 0.0,
        "elevation_mae_sum": 0.0,
        "angular_error_sum": 0.0,
        "azimuth_pred_sum": 0.0,
        "azimuth_pred_sq_sum": 0.0,
        "azimuth_target_sum": 0.0,
        "azimuth_target_sq_sum": 0.0,
        "azimuth_deg_count": 0,
        "error_count": 0,
        "steps": 0,
    }
    ...
    for step, batch in enumerate(loader):
        ...
        with torch.set_grad_enabled(train):
            ...
            loss, loss_dict = compute_losses(
                ...
                batch,
                loss_weights,
                azimuth_head_mode=args.azimuth_head_mode,
                azimuth_regression_loss=args.azimuth_regression_loss,
                front9_label_space=front9_label_space,
            )
        ...
        azimuth_pred_deg = get_azimuth_predictions_deg(outputs, args, front9_label_space)
        elevation_pred = outputs["elevation_logits"].argmax(dim=-1) if "elevation_logits" in outputs else None
        if args.azimuth_head_mode in {"front_regression", "front9_classification"}:
            target_azimuth_raw_deg = batch["azimuth_target"].float()
            target_azimuth_deg = azimuth_raw_to_signed_front_deg(batch["azimuth_target"])
        else:
            target_azimuth_raw_deg = batch.get("azimuth_target_deg", batch["azimuth_target"].float())
            target_azimuth_deg = azimuth_to_signed_deg(target_azimuth_raw_deg)

        if step == 0 and train and azimuth_pred_deg is not None and elevation_pred is not None:
            print("\n[TRAIN DEBUG - PRED]")
            if args.azimuth_head_mode == "front_regression":
                print("pred azimuth deg:", azimuth_pred_deg[:20])
                print(
                    "[DEBUG] azimuth regression mean/std:",
                    azimuth_pred_deg.mean().item(),
                    azimuth_pred_deg.std(unbiased=False).item(),
                )
            elif args.azimuth_head_mode in {"full360_regression", "full360_sincos_regression"}:
                print("pred azimuth sin/cos:", outputs["azimuth_regression"][:20])
                print("pred azimuth deg:", azimuth_pred_deg[:20])
                print(
                    "[DEBUG] azimuth full360 regression mean/std:",
                    azimuth_pred_deg.mean().item(),
                    azimuth_pred_deg.std(unbiased=False).item(),
                )
            elif args.azimuth_head_mode == "front9_classification":
                pred_front_class = outputs["azimuth_front9_logits"].argmax(dim=-1)
                print("pred front9 class:", pred_front_class[:20])
                print("pred azimuth deg:", azimuth_pred_deg[:20])
                print(
                    "[DEBUG] azimuth front9 logits mean/std:",
                    outputs["azimuth_front9_logits"].mean().item(),
                    outputs["azimuth_front9_logits"].std().item(),
                )
            else:
                print("pred azimuth:", azimuth_pred_deg[:20].long())
                print(
                    "[DEBUG] azimuth logits mean/std:",
                    outputs["azimuth_logits"].mean().item(),
                    outputs["azimuth_logits"].std().item(),
                )
            print("pred elevation:", elevation_pred[:20])
            print(
            ...
            if hasattr(model.heads, "azimuth_front9_head") and model.heads.azimuth_front9_head is not None:
                az_front_weight = model.heads.azimuth_front9_head.proj.weight
                print(
                    "[DEBUG] azimuth_front9_head weight mean/std:",
                    az_front_weight.mean().item(),
                    az_front_weight.std().item(),
                )
            if hasattr(model.heads, "azimuth_regression_head") and model.heads.azimuth_regression_head is not None:
                az_reg_weight = model.heads.azimuth_regression_head.proj.weight
                print(
                    "[DEBUG] azimuth_regression_head weight mean/std:",
                    az_reg_weight.mean().item(),
                    az_reg_weight.std().item(),
                )
            if hasattr(model.heads, "elevation_head") and model.heads.elevation_head is not None:
        ...
        if azimuth_pred_deg is not None:
            if args.azimuth_head_mode == "front_regression":
                rounded_pred_class = signed_deg_to_front9_class(azimuth_pred_deg, front9_label_space)
                rounded_target_class = azimuth_raw_to_front9_class(batch["azimuth_target"], front9_label_space)
                rounded_match = (rounded_pred_class == rounded_target_class)
                stats["azimuth_correct"] += rounded_match.sum().item()
                stats["azimuth_rounded_front9_correct"] += rounded_match.sum().item()
                stats["azimuth_rounded_front9_count"] += batch_size
                azimuth_error = (azimuth_pred_deg - target_azimuth_deg).abs()
                azimuth_pred_signed = azimuth_pred_deg
            elif args.azimuth_head_mode == "front9_classification":
                pred_front_class = outputs["azimuth_front9_logits"].argmax(dim=-1)
                target_front_class = azimuth_raw_to_front9_class(batch["azimuth_target"], front9_label_space)
                front_match = (pred_front_class == target_front_class)
                stats["azimuth_correct"] += front_match.sum().item()
                stats["azimuth_front9_correct"] += front_match.sum().item()
                stats["azimuth_front9_count"] += batch_size
                azimuth_error = (azimuth_pred_deg - target_azimuth_deg).abs()
                azimuth_pred_signed = azimuth_pred_deg
            elif args.azimuth_head_mode in {"full360_regression", "full360_sincos_regression"}:
                rounded_pred = azimuth_pred_deg.round().long() % 360
                stats["azimuth_correct"] += (rounded_pred == batch["azimuth_target"]).sum().item()
                azimuth_error = circular_azimuth_error_deg(azimuth_pred_deg, target_azimuth_raw_deg)
                azimuth_pred_signed = azimuth_to_signed_deg(azimuth_pred_deg)
            else:
                azimuth_pred = azimuth_pred_deg.long()
                stats["azimuth_correct"] += (azimuth_pred == batch["azimuth_target"]).sum().item()
                azimuth_error = circular_azimuth_error_deg(azimuth_pred_deg, target_azimuth_raw_deg)
                azimuth_pred_signed = azimuth_to_signed_deg(azimuth_pred_deg)
            stats["azimuth_count"] += batch_size
            stats["azimuth_mae_sum"] += azimuth_error.sum().item()
            stats["azimuth_pred_sum"] += azimuth_pred_signed.sum().item()
            stats["azimuth_pred_sq_sum"] += (azimuth_pred_signed ** 2).sum().item()
            stats["azimuth_target_sum"] += target_azimuth_deg.sum().item()
            stats["azimuth_target_sq_sum"] += (target_azimuth_deg ** 2).sum().item()
            stats["azimuth_deg_count"] += batch_size
        ...
        if azimuth_pred_deg is not None and elevation_pred is not None:
            angular_error = angular_error_deg(
                azimuth_pred_deg,
                elevation_pred.float(),
        ...
        last_debug["azimuth_target_deg"] = target_azimuth_deg[:10].detach().cpu().tolist()
        last_debug["elevation_target"] = batch["elevation_target"][:10].detach().cpu().tolist()
        if azimuth_pred_deg is not None:
            last_debug["pred_azimuth_deg"] = azimuth_pred_deg[:10].detach().cpu().tolist()
            if args.azimuth_head_mode == "front9_classification":
                last_debug["pred_front9_class"] = (
                    outputs["azimuth_front9_logits"].argmax(dim=-1)[:10].detach().cpu().tolist()
                )
                last_debug["target_front9_class"] = (
                    azimuth_raw_to_front9_class(batch["azimuth_target"], front9_label_space)[:10]
                    .detach()
                    .cpu()
                    .tolist()
                )
            if args.azimuth_head_mode == "front_regression":
                last_debug["pred_rounded_front9_class"] = (
                    signed_deg_to_front9_class(azimuth_pred_deg, front9_label_space)[:10]
                    .detach()
                    .cpu()
                    .tolist()
                )
            if args.azimuth_head_mode in {"full360_regression", "full360_sincos_regression"}:
                last_debug["pred_azimuth_sincos"] = (
                    outputs["azimuth_regression"][:10].detach().cpu().tolist()
                )
                last_debug["pred_azimuth"] = azimuth_pred_deg[:10].round().detach().cpu().long().tolist()
            if args.azimuth_head_mode == "full360_classification":
                last_debug["pred_azimuth"] = azimuth_pred_deg[:10].detach().cpu().long().tolist()
        if elevation_pred is not None:
            last_debug["pred_elevation"] = elevation_pred[:10].detach().cpu().tolist()
    ...
    pred_azimuth_mean, pred_azimuth_std = summarize_mean_std(
        stats["azimuth_pred_sum"],
        stats["azimuth_pred_sq_sum"],
        stats["azimuth_deg_count"],
    )
    target_azimuth_mean, target_azimuth_std = summarize_mean_std(
        stats["azimuth_target_sum"],
        stats["azimuth_target_sq_sum"],
        stats["azimuth_deg_count"],
    )
    ...
    return {
        ...
        "azimuth_front9_acc": stats["azimuth_front9_correct"] / max(stats["azimuth_front9_count"], 1),
        "azimuth_rounded_front9_acc": stats["azimuth_rounded_front9_correct"] / max(stats["azimuth_rounded_front9_count"], 1),
        ...
        "azimuth_mae_deg": stats["azimuth_mae_sum"] / max(stats["azimuth_count"], 1),
        ...
        "pred_azimuth_deg_mean": pred_azimuth_mean,
        "pred_azimuth_deg_std": pred_azimuth_std,
        "target_azimuth_deg_mean": target_azimuth_mean,
        "target_azimuth_deg_std": target_azimuth_std,
        "steps": stats["steps"],
...
def main():
    ...
    parser.add_argument(
        "--azimuth_head_mode",
        default="front9_classification",
        choices=[
            "full360_classification",
            "full360_regression",
    ...
    parser.add_argument("--azimuth_regression_range", type=float, default=45.0)
    parser.add_argument(
        "--azimuth_regression_loss",
        default="smoothl1",
        choices=["smoothl1", "huber"],
    )
    ...
    parser.add_argument("--lr_patch_embed", type=float, default=1e-5)
    parser.add_argument("--unfreeze_patch_embed", action="store_true", dest="unfreeze_patch_embed")
    parser.add_argument("--freeze_patch_embed", action="store_false", dest="unfreeze_patch_embed")
    parser.set_defaults(unfreeze_patch_embed=True)
    ...
    parser.add_argument(
        "--select_best_by",
        default="angular_error",
        choices=["angular_error", "vector_cosine", "azimuth_mae", "azimuth_loss", "loss"],
    )
    ...
    if args.azimuth_head_mode in {"front_regression", "front9_classification"}:
        front9_label_space = build_front9_label_space_from_manifest([args.train_json, args.val_json])

    azimuth_front_class_count = front9_label_space["num_classes"] if front9_label_space is not None else 9
    ...
    model = build_model(
        ...
        azimuth_head_mode=args.azimuth_head_mode,
        azimuth_regression_range=args.azimuth_regression_range,
        azimuth_front_class_count=azimuth_front_class_count,
    ).to(device)

    checkpoint_path = args.init_checkpoint or args.pretrained_backbone_ckpt
    if checkpoint_path:
    ...
    for epoch in range(args.epochs):
        ...
        summary = {
            ...
            "train_azimuth_front9_acc": train_stats["azimuth_front9_acc"],
            "train_azimuth_rounded_front9_acc": train_stats["azimuth_rounded_front9_acc"],
            ...
            "train_azimuth_mae_deg": train_stats["azimuth_mae_deg"],
            "train_elevation_mae": train_stats["elevation_mae"],
            "train_angular_error": train_stats["angular_error"],
            "train_pred_azimuth_deg_mean": train_stats["pred_azimuth_deg_mean"],
            "train_pred_azimuth_deg_std": train_stats["pred_azimuth_deg_std"],
            "train_target_azimuth_deg_mean": train_stats["target_azimuth_deg_mean"],
            "train_target_azimuth_deg_std": train_stats["target_azimuth_deg_std"],
            ...
            "val_azimuth_front9_acc": val_stats["azimuth_front9_acc"],
            "val_azimuth_rounded_front9_acc": val_stats["azimuth_rounded_front9_acc"],
            "val_elevation_acc": val_stats["elevation_acc"],
            "val_vector_cosine": val_stats["vector_cosine"],
            "val_azimuth_mae": val_stats["azimuth_mae"],
            "val_azimuth_mae_deg": val_stats["azimuth_mae_deg"],
            "val_elevation_mae": val_stats["elevation_mae"],
            "val_angular_error": val_stats["angular_error"],
            "val_pred_azimuth_deg_mean": val_stats["pred_azimuth_deg_mean"],
            "val_pred_azimuth_deg_std": val_stats["pred_azimuth_deg_std"],
            "val_target_azimuth_deg_mean": val_stats["target_azimuth_deg_mean"],
            "val_target_azimuth_deg_std": val_stats["target_azimuth_deg_std"],
    ...
    metrics_summary = {
        "config": {
            ...
            "azimuth_regression_range": args.azimuth_regression_range,
            "azimuth_regression_loss": args.azimuth_regression_loss,
            "lr_patch_embed": args.lr_patch_embed,
    ...
    if args.debug_overfit_one_sample and history:
        ...
        debug_payload = {
            ...
            "final_target_azimuth_deg": train_stats["last_debug"].get("azimuth_target_deg", []),
            ...
            "final_predicted_azimuth_deg": train_stats["last_debug"].get("pred_azimuth_deg", []),
        ...
        if (
            ...
            print("[WARNING] Localization heads still not adapting; inspect logits/token statistics.")
