...
RUN_NAME="${RUN_NAME:-full_hm3d_losnlos_100k_balanced_full360_${STEM_VARIANT}_reg_last${UNFREEZE_LAST_N_BLOCKS}_patchunfreeze}"
...
if [ ! -f "${TRAIN_JSON}" ] || [ ! -f "${VAL_JSON}" ]; then
  echo "[stage15-hm3d] full360 manifests missing, building them first"
...
"${SCRIPT_DIR}/run_stage15_full_experiment.sh" \
  ...
  --recipe_name full360_sincos_patchunfreeze_slow_recipe \
  --lr_stem 1e-4 \
  --lr_adapter 1e-4 \
  --lr_heads 5e-4 \
  --lr_patch_embed 1e-5 \
