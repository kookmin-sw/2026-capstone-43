...
def build_foa_stem(in_chans, hidden_channels_list, out_chans, norm_type="groupnorm", act="gelu"):
    if norm_type != "groupnorm":
        raise ValueError(f"Unsupported FOA stem norm_type: {norm_type}")
    if act != "gelu":
        raise ValueError(f"Unsupported FOA stem activation: {act}")
    if not hidden_channels_list:
        raise ValueError("FOA stem must include at least one hidden convolution block")

    layers = [nn.GroupNorm(1, in_chans)]
    prev_chans = in_chans
    for hidden_chans in hidden_channels_list:
        layers.extend(
            [
                conv3x3(prev_chans, hidden_chans),
                nn.BatchNorm2d(hidden_chans),
                nn.GELU(),
            ]
        )
        prev_chans = hidden_chans

    layers.extend(
        [
            conv3x3(prev_chans, out_chans),
...
def resolve_foa_stem_config(variant, default_hidden, hidden_override=0, out_override=0):
    if variant not in FOA_STEM_VARIANTS:
        raise ValueError(f"Unsupported FOA stem variant: {variant}")

    variant_config = FOA_STEM_VARIANTS[variant]
    hidden_channels = list(variant_config["hidden_channels"])
    if hidden_override > 0:
        hidden_channels = [hidden_override] * len(hidden_channels)
    elif variant == "baseline":
        hidden_channels = [default_hidden] * len(hidden_channels)
    out_chans = out_override or variant_config["out_channels"]
    return hidden_channels, out_chans
...
class SpatialASTFOABackbone(_VisionTransformer):
    def __init__(
        ...
        self.foa_stem_hidden_channels, self.foa_stem_out_channels = resolve_foa_stem_config(
        ...
        self.foa_stem_depth = len(self.foa_stem_hidden_channels) + 1
        ...
        self.foa_native_stem = build_foa_stem(
            ...
            hidden_channels_list=self.foa_stem_hidden_channels,
