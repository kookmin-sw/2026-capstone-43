...
class SpatialASTFOABackbone(_VisionTransformer):
    def __init__(
        ...
        self.foa_native_stem = nn.Sequential(
            nn.GroupNorm(1, self.foa_feature_extractor.channels_num),
            conv3x3(self.foa_feature_extractor.channels_num, foa_stem_channels),
            nn.BatchNorm2d(foa_stem_channels),
            nn.GELU(),
            conv3x3(foa_stem_channels, 1),
            nn.BatchNorm2d(1),
            nn.GELU(),
        )
