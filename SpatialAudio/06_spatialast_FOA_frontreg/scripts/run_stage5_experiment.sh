...
MANIFEST_ROOT="${PROJECT_ROOT}/manifests_stage5"
TRAIN_MANIFEST="${MANIFEST_ROOT}/train_stage5_subset_1200.json"
VAL_MANIFEST="${MANIFEST_ROOT}/val_stage5_subset_200.json"

mkdir -p "${OUTPUT_DIR}"

if [ ! -f "${TRAIN_MANIFEST}" ] || [ ! -f "${VAL_MANIFEST}" ]; then
  echo "[stage5] subset manifests missing, building them first"
  "${PYTHON_BIN}" "${PROJECT_ROOT}/tools/build_stage5_subset.py"
fi
