...
OUTPUT_ROOT = ROOT / "manifests_stage14"
...
def write_summary(path, split_stats):
    with open(path, "w") as f:
        f.write("# Stage-14 Subset Summary\n\n")
        f.write("Subset goal: compare front-cone azimuth supervision modes on top of the Stage-13 conv stem variants.\n")
        f.write("Sampling mode: azimuth-balanced, with elevation/geometry stratification inside each azimuth.\n")
        f.write("Current front-cone support: 320, 330, 340, 350, 0, 10, 20, 30, 40 degrees.\n\n")
...
def main():
    ...
    train_path = output_dir / f"train_stage14_subset_{args.train_size}.json"
    val_path = output_dir / f"val_stage14_subset_{args.val_size}.json"
    write_json(train_path, train_stats["data"])
    write_json(val_path, val_stats["data"])

    summary_path = output_dir / "stage14_subset_summary.md"
