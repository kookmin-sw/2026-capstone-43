...
OUTPUT_ROOT = ROOT / "manifests_stage5"
...
def allocate_evenly(keys, target_size, capacity_map):
    ...
    for idx, key in enumerate(sorted(keys)):
        ...
        if wanted > capacity:
            raise ValueError(
                f"cannot allocate {wanted} samples to azimuth {key}; capacity is {capacity}"
            )
...
def allocate_counts(groups, target_size):
    ...
    if target_size < len(strata):
        raise ValueError(
            f"target_size={target_size} is smaller than number of strata={len(strata)}"
        )
    ...
    ideal_extra = {
        key: remaining * extra_capacity[key] / total_extra_capacity
        for key in strata
    }
    extra_floor = {
...
def write_summary(path, split_stats):
    with open(path, "w") as f:
        f.write("# Stage-5 Subset Summary\n\n")
        f.write("Subset goal: fast Stage-5 candidate comparison, not final reporting.\n")
...
def main():
    ...
    train_path = output_dir / f"train_stage5_subset_{args.train_size}.json"
    val_path = output_dir / f"val_stage5_subset_{args.val_size}.json"
    write_json(train_path, train_stats["data"])
    write_json(val_path, val_stats["data"])

    summary_path = output_dir / "stage5_subset_summary.md"
