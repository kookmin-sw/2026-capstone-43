...
OUTPUT_ROOT = ROOT / "manifests_stage12"
...
def write_summary(path, split_stats):
    with open(path, "w") as f:
        f.write("# Stage-12 Subset Summary\n\n")
        f.write("Subset goal: compare FOA-native stem bottleneck variants under the same slow recipe.\n")
...
def main():
    ...
    train_path = output_dir / f"train_stage12_subset_{args.train_size}.json"
    val_path = output_dir / f"val_stage12_subset_{args.val_size}.json"
    write_json(train_path, train_stats["data"])
    write_json(val_path, val_stats["data"])

    summary_path = output_dir / "stage12_subset_summary.md"
