...
def front_nearest10_accuracy(pred_deg, target_deg):
    pred_bucket = (pred_deg / 10.0).round().clamp(-4.0, 4.0) * 10.0
    return (pred_bucket == target_deg).sum().item()


def get_azimuth_predictions_deg(outputs, args):
...
def prepare_debug_targets(batch, train, step):
    ...
    if step == 0 and train:
        ...
        if torch.all((azimuth_target <= 45) | (azimuth_target >= 315)):
            signed_front = azimuth_to_signed_front_deg(azimuth_target, strict_support=True)
...
def run_epoch(model, loader, optimizer, device, loss_weights, args, train=True, max_steps=0):
    ...
    for step, batch in enumerate(loader):
        ...
        azimuth_pred_deg = get_azimuth_predictions_deg(outputs, args)
        ...
        if args.azimuth_head_mode == "front_regression":
            target_azimuth_deg = azimuth_to_signed_front_deg(batch["azimuth_target"], strict_support=True)
        elif torch.all((batch["azimuth_target"] <= 45) | (batch["azimuth_target"] >= 315)):
            target_azimuth_deg = azimuth_to_signed_front_deg(batch["azimuth_target"], strict_support=False)
        else:
            target_azimuth_deg = azimuth_to_signed_deg(batch["azimuth_target"])
        ...
        if azimuth_pred_deg is not None:
            if args.azimuth_head_mode == "front_regression":
                stats["azimuth_correct"] += front_nearest10_accuracy(azimuth_pred_deg, target_azimuth_deg)
                ...
                azimuth_error = circular_azimuth_error_deg(azimuth_pred_deg, batch["azimuth_target"].float())
...
def main():
    ...
    parser.add_argument(
        "--azimuth_head_mode",
        default="full360_classification",
        choices=["full360_classification", "front_regression"],
    )
