...
history_file_path = '/home/yuchaehee/long_term_memory_project/my_long_term_mem_project/history_tasks/task_history.json'
...
short_term_memory_path = '/home/yuchaehee/long_term_memory_project/my_long_term_mem_project/memory/memory3.json'

robots = [{'name': 'fetch1', 'skills': ['GoToObject']}]
...
def save_task():
    # 여기서 execute_LLM_plan의 run_scripts() 호출
    task = task_entry.get().strip()
    if task:
        # 이전 실행에서 q/esc로 멈췄을 수 있으니 새 task 시작 전에 리셋
        execute_LLM_plan.task_over = False
        ...
        with open('/home/yuchaehee/long_term_memory_project/my_long_term_mem_project/prompts/instruction.txt', 'w', encoding='utf-8') as file:
            file.write(content)
        ...
        planner_ok = execute_LLM_plan.run_scripts()
        if not planner_ok:
            messagebox.showerror("Error", "Task planner failed. Check terminal logs.")
            return
...
def pump_action_queue():
    # Habitat env.step는 메인 스레드에서만 처리 (EGL_BAD_ACCESS 방지)
    try:
        execute_LLM_plan.exec_actions_tick()
    except Exception as e:
        print(f"[GUI] action tick error: {e}")
    root.after(10, pump_action_queue)
