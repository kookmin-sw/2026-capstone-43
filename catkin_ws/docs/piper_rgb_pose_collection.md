# Piper RGB + Camera Pose 수집

이 문서는 Piper에 달린 RGB/RGB-D 카메라에서 `/camera/color/image_raw`와 camera pose를 함께 저장하는 방법을 정리한다.
...
```text
filename,image_stamp,tf_stamp,tx,ty,tz,qx,qy,qz,qw
```
